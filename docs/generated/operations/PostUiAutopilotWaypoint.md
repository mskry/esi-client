...
- Stable ID: `PostUiAutopilotWaypoint`
- HTTP: `POST /ui/autopilot/waypoint`
- Domain method: `client.userInterface.postUiAutopilotWaypoint(options)`
- Generic call: `client.callOperation("PostUiAutopilotWaypoint", arguments, callOptions?)`
...
const data = await client.userInterface.postUiAutopilotWaypoint({ addToBeginning: true, clearOtherWaypoints: true, destinationId: 12345 });
```
...
const arguments_: CallOperationArguments<'PostUiAutopilotWaypoint'> = { query: { "add_to_beginning": true, "clear_other_waypoints": true, "destination_id": 12345 } };
...
| destination_id | query | yes | integer (int64) |  |
...
- Metadata result: `client.userInterface.withMetadata().postUiAutopilotWaypoint(...)` returns `EsiResponse<T>`.
