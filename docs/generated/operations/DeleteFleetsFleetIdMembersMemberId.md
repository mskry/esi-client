...
- Stable ID: `DeleteFleetsFleetIdMembersMemberId`
- HTTP: `DELETE /fleets/{fleet_id}/members/{member_id}`
- Domain method: `client.fleets.deleteFleetsFleetIdMembersMemberId(fleetId, memberId, options?)`
- Generic call: `client.callOperation("DeleteFleetsFleetIdMembersMemberId", arguments, callOptions?)`
...
const data = await client.fleets.deleteFleetsFleetIdMembersMemberId(fleetId, memberId);
```
...
const arguments_: CallOperationArguments<'DeleteFleetsFleetIdMembersMemberId'> = { path: { "fleet_id": fleetId, "member_id": memberId } };
...
| member_id | path | yes | integer (int64) |  |
...
- Metadata result: `client.fleets.withMetadata().deleteFleetsFleetIdMembersMemberId(...)` returns `EsiResponse<T>`.
