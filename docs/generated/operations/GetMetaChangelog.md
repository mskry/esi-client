...
- Domain method: `client.meta.getMetaChangelog(options?)`
...
const data = await client.meta.getMetaChangelog();
```
...
- Metadata result: `client.meta.withMetadata().getMetaChangelog(...)` returns `EsiResponse<T>`.
