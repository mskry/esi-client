...
- Stable ID: `GetUniverseGraphicsGraphicId`
- HTTP: `GET /universe/graphics/{graphic_id}`
- Domain method: `client.universe.getUniverseGraphicsGraphicId(graphicId, options?)`
- Generic call: `client.callOperation("GetUniverseGraphicsGraphicId", arguments, callOptions?)`
...
const data = await client.universe.getUniverseGraphicsGraphicId(graphicId);
```
...
| --- | --- | --- | --- | --- |
| graphic_id | path | yes | integer (int64) |  |
...
- Metadata result: `client.universe.withMetadata().getUniverseGraphicsGraphicId(...)` returns `EsiResponse<T>`.
