...
- Stable ID: `GetUniverseTypesTypeId`
- HTTP: `GET /universe/types/{type_id}`
- Domain method: `client.universe.getUniverseTypesTypeId(typeId, options?)`
- Generic call: `client.callOperation("GetUniverseTypesTypeId", arguments, callOptions?)`
...
const data = await client.universe.getUniverseTypesTypeId(typeId);
```
...
- Metadata result: `client.universe.withMetadata().getUniverseTypesTypeId(...)` returns `EsiResponse<T>`.
