...
- Stable ID: `GetParagonHubSkinrCorporations`
- HTTP: `GET /paragon-hub/skinr/corporations/{corporation_id}`
- Domain method: `client.paragonHub.getParagonHubSkinrCorporations(corporationId, options?)`
- Generic call: `client.callOperation("GetParagonHubSkinrCorporations", arguments, callOptions?)`
...
const data = await client.paragonHub.getParagonHubSkinrCorporations(corporationId);
```
...
| --- | --- | --- | --- | --- |
| corporation_id | path | yes | reference #/components/schemas/CorporationID | The ID of the corporation the listings are targeted at |
...
- Metadata result: `client.paragonHub.withMetadata().getParagonHubSkinrCorporations(...)` returns `EsiResponse<T>`.
