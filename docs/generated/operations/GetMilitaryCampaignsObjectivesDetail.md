...
- Domain method: `client.militaryCampaigns.getMilitaryCampaignsObjectivesDetail(campaignId, objectiveId, options?)`
...
const data = await client.militaryCampaigns.getMilitaryCampaignsObjectivesDetail(campaignId, objectiveId);
```
...
- Metadata result: `client.militaryCampaigns.withMetadata().getMilitaryCampaignsObjectivesDetail(...)` returns `EsiResponse<T>`.
