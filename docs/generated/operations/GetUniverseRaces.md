...
- Domain method: `client.universe.getUniverseRaces(options?)`
...
const data = await client.universe.getUniverseRaces();
```
...
- Metadata result: `client.universe.withMetadata().getUniverseRaces(...)` returns `EsiResponse<T>`.
