...
- Domain method: `client.universe.getUniverseRegionsRegionId(regionId, options?)`
...
const data = await client.universe.getUniverseRegionsRegionId(regionId);
```
...
- Metadata result: `client.universe.withMetadata().getUniverseRegionsRegionId(...)` returns `EsiResponse<T>`.
