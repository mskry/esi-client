<!--
@generated by @evespace/esi-client.
Compatibility date: 2026-08-18.
Specification SHA-256: d109f3a545525dd98ac0d8237c7838b3bfcb30cb1d971166959b633bac22d599.
DO NOT EDIT.
-->

# Authenticated operation

Read a token from the environment and call an authenticated typed method.

- Operation: [`GetCharactersCharacterIdLocation`](../operations/GetCharactersCharacterIdLocation.md)
- Repository source: `examples/generated/authenticated.ts`

```ts
import { EsiClient } from '@evespace/esi-client';

const characterId = 90000001;

export async function getAuthenticatedCharacterLocation() {
  const client = new EsiClient({ token: requiredAccessToken() });
  return client.location.getCharactersCharacterIdLocation(characterId);
}

function requiredAccessToken(): string {
  const token = process.env.ESI_ACCESS_TOKEN;
  if (!token) throw new Error('Set ESI_ACCESS_TOKEN before making this authorized request.');
  return token;
}
```
